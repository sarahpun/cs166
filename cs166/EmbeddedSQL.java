...
public class EmbeddedSQL {
   ...
   public static void Query1(EmbeddedSQL esql){
      try{
         String query = "SELECT S.sname, COUNT(*) FROM parts P, suppliers S, catalog C WHERE P.pid = C.pid AND S.sid = C.sid GROUP BY S.sid;";
   ...
   public static void Query2(EmbeddedSQL esql){
      try{
         String query = "SELECT S.sname, COUNT(*) FROM parts P, suppliers S, catalog C WHERE P.pid = C.pid AND S.sid = C.sid\n GROUP BY S.sid HAVING COUNT(C.pid) >= 3;";
   ...
   public static void Query3(EmbeddedSQL esql){
      try{
         String query = "SELECT S.sid, S.sname, COUNT(*) FROM suppliers S, parts P, catalog C WHERE S.sid = C.sid AND P.pid = C.pid AND S.sid IN ( SELECT S.sid FROM suppliers S, parts P, catalog C \n WHERE S.sid = C.sid AND P.pid = C.pid AND P.color = 'Green' EXCEPT SELECT C.sid FROM suppliers S, parts P, catalog C WHERE C.pid = P.pid AND p.color != 'Green') AND S.sid = C.sid AND P.pid = C.pid GROUP BY S.sid;";
   ...
   public static void Query4(EmbeddedSQL esql){
      try{
         String query = "SELECT S.sname, MAX(C.cost) FROM suppliers S, parts P, catalog C WHERE S.sid = C.sid AND P.pid = C.pid AND S.sid IN (SELECT S.sid FROM suppliers S, parts P, catalog C WHERE S.sid = C.sid AND P.pid = C.pid AND P.color = 'Green' INTERSECT Select S.sid FROM cuppliers S, parts P, catalog C WHERE S.sid = C.sid AND P.pid = C.pid AND P.color = 'Red') GROUP BY S.sid;";
